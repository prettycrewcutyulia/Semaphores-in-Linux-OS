...
sem_t *cashier_1_sem, *cashier_2_sem, *queue_sem, *for_cashier_1_sem;
sem_t *for_cashier_2_sem;
int num_customers;
...
void cleanup() {
    sem_destroy(cashier_1_sem); // Закрытие семафора для кассира 1
    sem_destroy(cashier_2_sem); // Закрытие семафора для кассира 2
    sem_destroy(queue_sem); // Закрытие семафора для очереди
    sem_destroy(for_cashier_1_sem); // Закрытие семафора для кассира 1
    sem_destroy(for_cashier_2_sem); // Закрытие семафора для кассира 2
...
// Создание семафора для кассы 1, начальное значение 0
    if (sem_init(cashier_1_sem, 0, 0) == -1) {
        perror("sem_init");
        exit(EXIT_FAILURE);
    }
    if (sem_init(cashier_2_sem, 0, 0) == -1) {
        perror("sem_init");
        exit(EXIT_FAILURE);
    }
    if (sem_init(queue_sem, 0, 1) == -1) {
        perror("sem_init");
        exit(EXIT_FAILURE);
    }
    if (sem_init(for_cashier_1_sem, 0, 0) == -1) {
        perror("sem_init");
        exit(EXIT_FAILURE);
    }
    if (sem_init(for_cashier_2_sem, 0, 0) == -1) {
        perror("sem_init");
        exit(EXIT_FAILURE);
    }

...
void cashier_behaviour() {
    while (customer_count_ptr[0] > 0) { // Пока в магазине есть покупатели
            wait_sem(cashier_1_sem); // Ожидание сигнала от покупателя, что он готов к оплате
        ...
        printf("Кассир номер %d занят. Пробивает продукты покупателя.\n",
               1); // Вывод сообщения о том, что кассир начал обслуживание покупателя
        ...
        signal_sem(for_cashier_1_sem); // Освобождение покупателя, что он может продолжить выполнение программы
    }
}